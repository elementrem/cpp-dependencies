...
rem because of EleDependencies.cmake in webthree-helper, which does an
...
rem libscrypt needs building for crossele.  Why don't we need it here?
...
rem We don't build libminiupnpc for crossele, so I'll have to guess on its layering for now.
...
rem secp256k1 needs building for crossele.  Why don't we need it here?
